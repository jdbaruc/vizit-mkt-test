...
const __iconUrl = (file) => {
  const id = 'icon_' + file.replace(/\.png$/, '').replace(/[^a-zA-Z0-9]/g, '_');
  return (window.__resources && window.__resources[id]) || (ICON_BASE + file);
};
const mk = (file) => <Icon src={__iconUrl(file)} />;
// Sized icon helper — use when you need a non-default size inline.
const IconAt = ({ name, size = 20, style, ...rest }) => (
  <Icon src={__iconUrl(name)} size={size} style={style} {...rest} />
);
