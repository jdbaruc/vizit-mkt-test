...
const LeftRail = ({ route = '/collections', onNavigate }) => {
  ...
  return (
    <aside style={{
      ...
      padding: '16px 0 16px', gap: 6, flexShrink: 0,
      position: 'sticky', top: 0, height: 'calc(100vh - 32px)', alignSelf: 'flex-start',
    }}>
...
const Crumbs = ({ onNavigate }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 4, height: 48, padding: '0 20px', borderBottom: '1px solid var(--border-secondary)' }}>
    ...
    >
      <span className="breadcrumb-btn__icon">{Icons.Layers}</span>
      <span className="breadcrumb-btn__label">Collections</span>
    ...
    >
      ...
      <span className="breadcrumb-btn__label">Test — shaving pillow shots</span>
    </button>
  </div>
);
...
/* ---------- Status bar (global footer) -----------------------------
   Persistent thin strip across the bottom of the entire app shell.
   Mirrors the visual language of IDE / desktop status strips: small
   copy, dot separators, inline glyphs. Left side anchors the active
   workspace ("Marspet"). Right side is a stream of background
   processes — each is its own segment so we can tint errors red
   without affecting siblings. Mounted ONCE in <App/> so it appears
   on every route automatically.
*/
const StatusSpinner = () => (
  <span style={{
    ...
    borderRadius: '50%',
    animation: 'vz-status-spin 0.8s linear infinite',
  }} />
);
...
const StatusBar = () => (
  <>
    <style>{`
      @keyframes vz-status-spin { to { transform: rotate(360deg); } }
      @keyframes vz-status-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.45; } }
    `}</style>
    <div
      role="status"
      aria-live="polite"
    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          width: 14, height: 14,
          background: '#e9d5f5', color: '#6b3a9a',
          display: 'grid', placeItems: 'center',
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 4px' }}>
        {/* Process 1: product pages with errors */}
        <button
          type="button"
        ...
        >
          <StatusSpinner />
          <span>Processing <span style={{ color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>37 / 50</span> product pages</span>
          <span style={{
            display: 'inline-flex', alignItems: 'center', gap: 4,
            color: 'var(--text-error-primary, #B42318)',
          }}>
        ...
        {/* Separator */}
        <span style={{ width: 3, height: 3, borderRadius: '50%', background: 'var(--gray-300)', margin: '0 2px' }} />

        {/* Process 2: AI Spark images */}
        <button
          type="button"
          title="View Spark image generation"
          style={{
            display: 'inline-flex', alignItems: 'center', gap: 6,
            height: '100%', padding: '0 10px',
            background: 'transparent', border: 0, cursor: 'pointer',
        ...
        >
          <span style={{
            ...
            animation: 'vz-status-pulse 1.6s ease-in-out infinite',
          }} />
          <span>Generating <span style={{ color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>12</span> Spark images</span>
        </button>
      </div>
    </div>
  </>
...
function App() {
  ...
  if (route.startsWith('/settings/tags')) {
    page = <TagManagerPage onBack={() => navigate('/collections')} route={route} onNavigate={navigate} />;
  } else if (route.startsWith('/library/product-pages/')) {
    page = <ProductPage route={route} onNavigate={navigate} />;
  } else if (route.startsWith('/library/pack-designs')) {
    page = <PackDesignPageV3 route={route} onNavigate={navigate} />;
  } else if (route.startsWith('/library/assets/')) {
    ...
    if (tile && tile.klass === 'Hero Image') {
      page = <HeroDetailsPage route={route} onNavigate={navigate} />;
    } else {
      page = <AssetDetailsPage route={route} onNavigate={navigate} />;
    }
  ...
  return (
    <>
      {/* Reserve room for the 32px status strip so fixed UIs (toasts,
          tweaks panel) and scrolled content don't collide with it. */}
      <div style={{ paddingBottom: 32 }}>{page}</div>
      <StatusBar />
    </>
  );
}
